...
#[test]
fn unavailable_hardware_statuses_are_conservative_and_do_not_change_screen() {
    let (screen, output) = replies(b"abc\x1b[?15n\x1b[?25n\x1b[5n");
    assert_eq!(output, b"\x1b[?11n\x1b[?21n\x1b[0n");
    let mut expected = Screen::new(8, 12).unwrap();
...
fn character_size_reports_use_current_screen_dimensions() {
    ...
    let mut output = Vec::new();
    Parser::new().advance_with_replies(&mut resized, b"\x1b[18t\x1b[19t", &mut |reply| {
        output.extend_from_slice(reply)
    });
    assert_eq!(output, b"\x1b[8;24;80t\x1b[9;24;80t");
...
fn malformed_unsupported_and_string_queries_produce_no_reply() {
    let (_, output) = replies(b"\x1b[n\x1b[0n\x1b[?n\x1b[?0n\x1b[?5n\x1b[?6;1n\x1b[?6:1n\x1b[?15;0n\x1b[?15:0n\x1b[?25;0n\x1b[?25:0n\x1b[5;6n\x1b[6:1n\x1b[999999999999999999999999n\x1b[?999999999999999999999999n\x1b]text\x1b[6n\x07\x1b[6\x18n");
