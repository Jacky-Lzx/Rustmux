...
The CLI now depends on our parser's supported subset. Queries beyond standard DSR and the [pane size/extent reports](status-replies.md), [terminal identity](device-attributes.md), [DECRQM](mode-queries.md), DECRQSS [cursor-style](cursor-shape.md) and [scroll-region](scrolling-regions.md) queries and the [Kitty keyboard query](kitty-keyboard.md), mouse extensions and full emoji shaping are not implemented. Programs requiring
